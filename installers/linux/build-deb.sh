...
PKG_DIR="$SCRIPT_DIR/deb/toneai-nux_${VERSION}_amd64"

rm -rf "$PKG_DIR" && mkdir -p "$PKG_DIR/usr/local/bin" "$PKG_DIR/DEBIAN" "$DIST_DIR"

cp "$ROOT_DIR/toneai-nux-linux-x64" "$PKG_DIR/usr/local/bin/toneai-nux"
chmod +x "$PKG_DIR/usr/local/bin/toneai-nux"
...
dpkg-deb --build "$PKG_DIR" "$DIST_DIR/toneai-nux_${VERSION}_amd64.deb"
echo "Built: $DIST_DIR/toneai-nux_${VERSION}_amd64.deb"

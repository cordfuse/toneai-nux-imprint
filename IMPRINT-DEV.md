...
```bash
GITHUB_TOKEN="" gh release edit vX.X.X --repo cordfuse/toneai-nux-imprint --notes "..."
```
...
| v1.2.0 | **Pipe installer** — `curl ... \| bash` (macOS/Linux) and `irm ... \| iex` (Windows) one-liners. Installs Bun if missing, fetches `toneai-nux.ts` from `releases/latest/download/`, runs the wizard. New release assets with stable filenames: `install.sh`, `install.ps1`, `toneai-nux.ts`, `toneai-source.zip`. **Wizard bug fix:** Step 2 now downloads the prebuilt source zip (production `dist/`) instead of `git clone`-ing the dev repo, which was landing users in dev mode (no ToneAI persona). Drops `installGit` from the wizard since git is no longer required. Wizard step 3 (npm install) also dropped — agent's first-launch runtime install handles deps. Pipe installer is added alongside existing paths in this release; cleanup of retired surfaces (setup wizard binaries, homebrew, OS installers) deferred to v1.3.0 once pipe installer is validated end-to-end. |
| v1.4.1 | **Pipe installer freeze fix.** The wizard (`setup/src/index.ts`) used `node:readline` `question()` for its prompts. `install.sh` correctly hands stdin to the wizard via `exec bun wizard </dev/tty`, but under Bun `readline`'s `question()` prints the prompt and never resolves on a redirected-TTY stdin — freezing every `curl … \| bash` install at the Step 2 "Where to install?" prompt (the first prompt that always fires when Node + an agent are present). Affected v1.2.0–v1.4.0 — the pipe installer never worked end-to-end via the public one-liner on a normal machine; the v1.2.0 "validated end-to-end" follow-up never happened. Fix: replaced `node:readline` with Bun's native `prompt()`, which reads the controlling TTY correctly. 3-line change, removes the `readline` import. **Release reminder:** jsdelivr caches `@latest` aggressively — purge `cdn.jsdelivr.net/gh/cordfuse/toneai-nux-imprint@latest/{install.sh,setup/src/index.ts}` after the release tag or the broken wizard keeps serving. |
| v1.4.2 | **Release-asset naming drift fix.** `release.yml` derived the product-ZIP basename from `${{ github.event.repository.name }}`, so the GitHub repo rename (toneai-nux → toneai-nux-imprint) silently renamed the ZIPs to `toneai-nux-imprint-*.zip` while the Bun-compiled binaries, pkg/deb/rpm, and brew formula stayed `toneai-nux-*` (those are hardcoded). Result: a half-renamed v1.4.1 release surface, and the auto-generated release body told users to download `toneai-nux-desktop-v1.4.1.zip` while the actual asset was `toneai-nux-imprint-desktop-v1.4.1.zip` (broken manual-download instruction). Pinned `REPO_NAME="toneai-nux"` (literal) in the three packaging steps for consistency. **Superseded by v1.4.3** — the decision was reversed: full rename instead of preserve. |
| v1.4.3 | **Full artifact rename → `toneai-nux-imprint-*`.** Reverses v1.4.2's preserve-the-old-name decision per an explicit, multiply-confirmed call: every distributable artifact + the brew command is renamed. **Renamed:** product ZIPs (`toneai-nux-imprint-{desktop-,}v*.zip`), `toneai-nux-imprint-source.zip`, `toneai-nux-imprint.ts` (staged wizard asset), Bun binaries (`toneai-nux-imprint-{linux-x64,win-x64.exe,mac-arm64,mac-x64}`), Inno `.exe`, `.pkg`/`.deb`/`.rpm` (incl. installed `/usr/local/bin` command, deb `Package:`, rpm spec name, pkg identifier `com.cordfuse.toneai-nux-imprint`), Homebrew formula (`toneai-nux-imprint.rb`, class `ToneaiNuxImprint`, `brew install toneai-nux-imprint`), `package.json`/`setup/package.json` names, `install.{sh,ps1}` internal `$REPO`/wizard-asset URLs. **Deliberately NOT renamed:** `install.sh`/`install.ps1` filenames themselves (generic bootstrap; the jsdelivr/release URL path already carries `toneai-nux-imprint`, so renaming would duplicate the name and break the public one-liner for zero gain); persona/app prose (product is conceptually "ToneAI"). Breaking change accepted: existing `brew install toneai-nux` users need re-install (tap alias added to soften). Old install.sh @latest on jsdelivr keeps the v1.4.1 readline fix; only the asset names changed — purge jsdelivr if the release-body/download names must update immediately. |
| v1.4.4 | **Straggler cleanup — the rename's last mile.** Removes the remaining off-spec `toneai-nux`/`toneai-nux-qr` references that v1.4.3 left behind. (1) `imprint/SESSION.md` auto-update `repo:` pointer → `toneai-nux-imprint` (was stale old name; GitHub-redirected but wrong). (2) `install.{sh,ps1}` wizard *scratch* dir `~/.toneai-nux` → `~/.toneai-nux-imprint` (ephemeral install cache, not user data; prior installs leave a harmless empty `~/.toneai-nux`). (3) **User-data dir `~/.imprint/toneai-nux-qr/` → `~/.imprint/toneai-nux-imprint/`** across WELCOME.md/MEMORY.md/IMPRINT-USER.md + the Homebrew `Cellar/toneai-nux-qr` check (formula is now `toneai-nux-imprint`). This dir held existing users' device/instrument/output-folder/preset-history/memory and was named after a *dead sibling-product name* — the single worst straggler. **Migrated, not orphaned:** new `Step 0` in WELCOME.md runs a silent, idempotent one-time `mv` (bash + PowerShell) of the old dir to the new on first launch — zero data loss. **Kept (not stragglers):** IMPRINT-DEV.md changelog entries for v1.2.0/v1.4.1 still name the old shipped artifacts — that's accurate history, rewriting it would be lying about what shipped. Step 0 is an app-engine behavior change — user-mode test the migration before relying on it. |
...
| v1.6.2 | **Binaries retired — pure-Node distribution.** Dropped the entire compiled-binary release surface: the four Bun-compiled setup-wizard binaries, the deb/rpm/pkg/Inno OS installers, the Homebrew formula update, the `curl \| bash` / `irm \| iex` pipe installers, and the `dist` branch push. `release.yml` is now a single Node job (mirrors `toneai-katana-imprint`): `npm ci` → `node src/build.js` → zip `dist/` and `dist-skill/` → GitHub release. Two assets only: `toneai-nux-skill-v*.zip` (~600 KB, the Skill — the one asset anyone actually downloaded) and `toneai-nux-imprint-v*.zip` (~200 KB desktop bundle). Removed `setup/`, `installers/`, `install.sh`, `install.ps1`. Desktop path is now "install Node.js, extract the zip, open in an agent CLI" — the QR generator is plain JS on a bare Node. Motivation: the binary matrix was ~460 MB per release at ⬇0 downloads. **Not touched:** `WELCOME.md` self-update flow still guards against a Homebrew-managed install — legacy `brew install` users from ≤v1.6.1 persist, and the guard keeps the ZIP self-updater from clobbering them. |
